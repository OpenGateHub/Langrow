...
import { getProfessorProfilebyString, getProfessorProfiles } from "./profile";
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (filter) {
      const data = await getProfessorProfilebyString(filter);
      ...
      const { data, total } = await getProfessorProfiles(page, limit);
      return NextResponse.json(
        {
          message: "Consulta exitosa",
          page,
...
export async function DELETE(req: Request) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json(
        {
          message: "Error al borrar un registro en la base de datos",
          error: error.message,
        },
        { status: 500 }
      );
